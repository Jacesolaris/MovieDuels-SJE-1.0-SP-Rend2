...
public:
	...
	void PlayEffect(int id, vec3_t origin, vec3_t axis[3], int boltInfo = -1, int ent_num = -1, bool isPortal = false,
	...
	// builds arbitrary perp. right vector, does a cross product to define up
	void PlayEffect(const char* file, vec3_t origin, vec3_t axis[3], int boltInfo, int ent_num, bool isPortal = false,
		int i_loop_time = false, bool is_relative = false);
