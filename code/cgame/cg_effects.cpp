...
void CG_PlayEffectBolted(const char* fx_name, const int modelIndex, const int bolt_index, const int ent_num,
	vec3_t origin,
	const int i_loop_time, const bool is_relative)
{
	...
	gi.G2API_AttachEnt(&boltInfo,
		&g_entities[ent_num].ghoul2[modelIndex],
		bolt_index,
		ent_num,
		modelIndex);
...
void CG_PlayEffectIDBolted(const int fx_id, const int modelIndex, const int bolt_index, const int ent_num,
	vec3_t origin,
	const int i_loop_time, const bool is_relative)
{
	const char* fx_name = CG_ConfigString(CS_EFFECTS + fx_id);
	CG_PlayEffectBolted(fx_name, modelIndex, bolt_index, ent_num, origin, i_loop_time, is_relative);
}
